<script lang="ts">
  ...
  import { page, task } from "../store/ui";
...
<main>
  ...
  <Subtitle size="1.25rem" subtitle="Go to Screen" />
  <div class="actions">
    <Button text="Main Page" on:click={() => goToScreen("main")} small={true} />
    <Button
      text="Meeting Room"
      on:click={() => goToScreen("meeting")}
      small={true}
    />
    <Button text="Wires Task" on:click={() => goToTask("wires")} small={true} />
    <Button
      text="Calibration Task"
      on:click={() => goToTask("calibration")}
      small={true}
    />
    <Button
      text="Switches Task"
      on:click={() => goToTask("switches")}
      small={true}
    />
  </div>
